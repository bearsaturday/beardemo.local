...
class SmartyValidate
{
     ...
     */
    public function connect(&$smarty, $reset = false)
    {
        if (self::is_valid_smarty_object($smarty)) {
            self::_object_instance('Smarty', $smarty);
     ...
     */
    public function is_valid_smarty_object(&$smarty_obj)
    {
     ...
     */
    public function register_form($form, $reset = false)
    {
     ...
     * @param string $form the name of the form being validated
     */
    public function is_registered_form($form = SMARTY_VALIDATE_DEFAULT_FORM)
    {
    ...
    public function _failed_fields(&$formvars, $form = SMARTY_VALIDATE_DEFAULT_FORM, $revalidate = false)
    {
        ...
        foreach ($_sess as $_key => $_val) {
            ...
            if (is_array($_ret) && in_array($_full_field, $_ret, true)) {
                ...
                foreach ($_trans_names as $_trans_name) {
                    ...
                    if (is_array($formvars[$_field]) && ! $_trans_on_array) {
                        if (isset($_field_key)) {
                            ...
                            if (($_new_val = self::_execute_transform($_trans_name, $formvars[$_field][$_field_key], $_sess[$_key], $formvars, $form)) !== false) {
                                $formvars[$_field][$_field_key] = $_new_val;
                            }
                        } else {
                            // apply to all keys
                            foreach ($formvars[$_field] as $_fv_key => $_fv_value) {
                                if (($_new_val = self::_execute_transform($_trans_name, $formvars[$_field][$_fv_key], $_sess[$_key], $formvars, $form)) !== false) {
                                    $formvars[$_field][$_x] = $_new_val;
                                }
                            }
                        }
                    } else {
                        if (($_new_val = self::_execute_transform($_trans_name, $formvars[$_field], $_sess[$_key], $formvars, $form)) !== false) {
            ...
            if ((! isset($formvars[$_field]) && (! isset($_FILES[$_field])))
                ...
                if (is_array($formvars[$_field]) && ! $_criteria_on_array) {
                    if (isset($_field_key)) {
                        // only apply to given key
                        $_sess[$_key]['valid'] = self::_is_valid_criteria($_val['criteria'], $formvars[$_field][$_field_key], $_empty, $_sess[$_key], $formvars, $form);
                    } else {
                        // apply to all keys
                        foreach ($formvars[$_field] as $_fv_key => $_fv_value) {
                            if (! $_sess[$_key]['valid'] = self::_is_valid_criteria($_val['criteria'], $formvars[$_field][$_fv_key], $_empty, $_sess[$_key], $formvars, $form)) {
                    ...
                    $_sess[$_key]['valid'] = self::_is_valid_criteria($_val['criteria'], $formvars[$_field], $_empty, $_sess[$_key], $formvars, $form);
                }
            }
    ...
    public function is_valid(&$formvars, $form = SMARTY_VALIDATE_DEFAULT_FORM)
    {
        ...
        $_failed_fields = self::_failed_fields($formvars, $form);
    ...
    public function register_object($object_name, &$object)
    {
        ...
        self::_object_instance($object_name, $object);
    }
     ...
     * @param string $func_name the function being registered
     */
    public function is_registered_object($object_name)
    {
        $_object = &self::_object_instance($object_name, $_dummy);
    ...
    {
        return self::_register_function('criteria', $name, $func_name, $form);
    }
     ...
     * @param string $func_name the function being registered
     */
    public function register_transform($name, $func_name, $form = SMARTY_VALIDATE_DEFAULT_FORM)
    {
        return self::_register_function('transform', $name, $func_name, $form);
    }
     ...
     */
    public function is_registered_criteria($name, $form = SMARTY_VALIDATE_DEFAULT_FORM)
    {
        if (! self::is_registered_form($form)) {
     ...
     * @param string $var the value being booleanized
     */
    public function is_registered_transform($name, $form = SMARTY_VALIDATE_DEFAULT_FORM)
    {
        if (! self::is_registered_form($form)) {
     ...
     */
    public function register_validator($id, $field, $criteria, $empty = false, $halt = false, $transform = null, $form = SMARTY_VALIDATE_DEFAULT_FORM)
    {
        if (! self::is_registered_form($form)) {
     ...
     * @param string $form      name of the form (optional)
     */
    public function set_transform($id, $transform, $form = SMARTY_VALIDATE_DEFAULT_FORM)
    {
        if (($_validator_key = self::is_registered_validator($id, $form)) === false) {
     ...
     */
    public function is_registered_validator($id, $form = SMARTY_VALIDATE_DEFAULT_FORM)
    {
        if (! self::is_registered_form($form)) {
     ...
     */
    public function unregister_validator($id, $form = SMARTY_VALIDATE_DEFAULT_FORM)
    {
        if (! self::is_registered_form($form)) {
            return false;
        }
     ...
     * @param string $form the name of the form being validated
     */
    public function set_page($page, $form = SMARTY_VALIDATE_DEFAULT_FORM)
    {
     ...
     */
    public function _execute_transform($name, $value, $params, &$formvars, $form)
    {
        if (self::is_registered_transform($name, $form)) {
            $_func_name = self::_get_registered_func_name('transform', $name, $form);
        } else {
        ...
        if (strpos($_func_name, '->') !== false) {
            ...
            $_object = &self::_object_instance($_object_name, $_dummy);
     ...
     * @param string $func_name the function being registered
     */
    public function _register_function($type, $name, $func_name, $form = SMARTY_VALIDATE_DEFAULT_FORM)
    {
        if (! self::is_registered_form($form)) {
     ...
     * @param string $form the form name
     */
    public function _get_registered_func_name($type, $name, $form)
    {
     ...
     */
    public function _is_valid_criteria($criteria, $value, $empty, &$params, &$formvars, $form)
    {
        if (self::is_registered_criteria($criteria, $form)) {
            $_func_name = self::_get_registered_func_name('criteria', $criteria, $form);
        } else {
